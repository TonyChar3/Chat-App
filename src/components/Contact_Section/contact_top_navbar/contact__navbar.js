    ...
    const handleAddon = async(e) => {
        ...
        try{
            ...
            if(name === "" || email === ""){
                ...
                const current_user_Ref = doc(db, "users", auth.currentUser.uid) // current logged in active user ref

                const contact_Ref = doc(db, 'users', added_contact.user_uid) // the contact that he want to add ref
